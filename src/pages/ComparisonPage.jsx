    ...
    <div className="container mt-4">
      ...
      <div className="row g-4 mb-4">
        <div className="col-md-6">
          ...
          <select className="form-select" onChange={(event) => fetchProductA(event.target.value)}>
            {!productA && <option value="Select a product">Select a product</option>}
            {products
              .filter((product) => !productB || product.id != productB.id)
              .map((product) => (
        ...
        <div className="col-md-6">
          <label htmlFor="second-product" className="form-label">
            Choose game
          </label>
          <select className="form-select" onChange={(event) => fetchProductB(event.target.value)}>
            {!productB && <option value="Select a product">Select a product</option>}
            {products
              .filter((product) => !productA || product.id != productA.id)
              .map((product) => (
                <option key={product.id} value={product.id}>
                  {product.title}
                </option>
